...
det_inst = CaffeNetFaceDetector.get_instance(input_type='uint8', zoom_factor=0.8, face_loc_resize=(1.6, 1.6))
img_names = sorted(os.listdir(src_dir))
face_locs = []
for i, name in enumerate(img_names):
    if '.jpg' in name:
        img = cv2.cvtColor(cv2.imread(os.path.join(src_dir, name)), cv2.COLOR_BGR2RGB)
        face_roi, face_loc = det_inst.extract_face_from_rgbImage(img, return_loc=True)
        ...
        cv2.imwrite(os.path.join(dst_dir, name.replace('.jpg','.png')), face_roi)
