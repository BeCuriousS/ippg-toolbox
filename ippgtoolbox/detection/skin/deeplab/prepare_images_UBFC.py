...
det_inst = CaffeNetFaceDetector.get_instance(input_type='uint8', zoom_factor=1, face_loc_resize=(1.2, 1.2))
vid_obj = UBFCVidReader(os.path.join(src_dir, 'vid.avi'))
face_locs = []
for i, frame in enumerate(vid_obj.reader_next()):
    face_roi, face_loc = det_inst.extract_face_from_rgbImage(frame, return_loc=True)
    ...
    cv2.imwrite(os.path.join(dst_dir, '{}.png'.format(str(i).zfill(5))), face_roi)
