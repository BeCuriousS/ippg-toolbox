...
flags.DEFINE_boolean('save_softmax_probabilities', False,
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
    ...
    #tf.logging.info('Start calc at ' + str(t1))
    ...
    #tf.logging.info('-----------Time difference is ' + str(t2-t1))
    ...
    for i in range(num_image):
        ...
        # tf.logging.info('-----------Time difference is ' + str(t3-t2))
    ...
    """
        # img_name = _PREDICTION_FORMAT % (image_id_offset + i)
        img_name = os.path.basename(image_names[i]).decode('utf-8') + '_segm'
        if FLAGS.save_softmax_probabilities == True:
            save_annotation.save_annotation(
                crop_semantic_prediction, save_dir,
                img_name, add_colormap=False,
...
def main(unused_argv):
    ...
    with tf.Graph().as_default():
        ...
        if FLAGS.save_softmax_probabilities == True:
            predictions = tf.cast(tf.math.round(tf.math.scalar_mul(
                255, predictions[common.OUTPUT_TYPE + '_prob'][:, :, :, 1])), tf.int32)
        else:
        ...
        for checkpoint_path in checkpoints_iterator:
            ...
            with tf.train.MonitoredSession(
                ...
                progress = progressbar.ProgressBar(
                    max_value=dataset.num_samples,
                    term_width=80,
                    widget_kwargs={'marker':'▒'}).start()
                ...
                while not sess.should_stop():
                    ...
                    # tf.logging.info('Visualizing batch %d', batch + 1)
                    # tf.logging.info(
                    #     20*'>'+' processing {:6d}/{}'.format(
                    #         batch, dataset.num_samples))
                    
                progress.finish()
                print('Finished computation with deeplab.')
...
if __name__ == '__main__':
    ...
    print('>>> Running deeplab on record data...')
    tf.app.run()
