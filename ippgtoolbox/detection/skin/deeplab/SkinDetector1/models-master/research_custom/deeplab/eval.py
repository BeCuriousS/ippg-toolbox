...
def main(unused_argv):
  ...
  with tf.Graph().as_default():
    ...
    ### insert: add class-wise and mean precision and recall as metrics
    predictions_tag = 'precision'
    for eval_scale in FLAGS.eval_scales:
      predictions_tag += '_' + str(eval_scale)
    if FLAGS.add_flipped_images:
      predictions_tag += '_flipped'

    metric_map['eval/%s_overall' % predictions_tag] = tf.metrics.precision(
    ...
    for c in range(num_classes):
      ...
      fp, fp_op = tf.metrics.false_positives(
          labels=one_hot_labels[:, c], predictions=one_hot_predictions[:, c],
          weights=weights)
      tp_fp_op = tf.group(tp_op, fp_op)
      precision = tf.where(tf.greater(tp, 0.0),
                     tp / (tp + fp),
                     tf.constant(np.NaN))
      metric_map['eval/%s' % predictions_tag_c] = (precision, tp_fp_op)

    predictions_tag = 'recall'
    for eval_scale in FLAGS.eval_scales:
      predictions_tag += '_' + str(eval_scale)
    if FLAGS.add_flipped_images:
      predictions_tag += '_flipped'

    metric_map['eval/%s_overall' % predictions_tag] = tf.metrics.recall(
        labels=labels, predictions=predictions, weights=weights)
    ...
    for c in range(num_classes):
      ...
      fn, fn_op = tf.metrics.false_negatives(
          labels=one_hot_labels[:, c], predictions=one_hot_predictions[:, c],
          weights=weights)
      tp_fn_op = tf.group(tp_op, fn_op)
      recall = tf.where(tf.greater(tp, 0.0),
                     tp / (tp + fn),
                     tf.constant(np.NaN))
      metric_map['eval/%s' % predictions_tag_c] = (recall, tp_fn_op)
    ...
    """
    contrib_training.evaluate_once(
